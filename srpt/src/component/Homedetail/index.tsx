...
export default function Homedetail({ className, type }: Props) {
    ...
    const getData = () => {
        ...
        let timer = setInterval(() => {
            if (typeof type === 'number' && dataList && dataList[0]) {
                switch (type) {
                    case 0: getEquipmentData(first); break;
                    case 1: getLoadData(first); break;
                    case 2: getElectricData(first); break;
                }
            }
            if (first) {
                first = false
    ...
    React.useEffect(() => {
        console.log(type, 'type', dataList, 'dataList');
    ...
    const getEquipmentData = (first: boolean) => {
        if (first) {
            setLoad(true)
    ...
    const getLoadData = (first: boolean) => {
        if (first) {
            setLoad(true)
    ...
    const getElectricData = (first: boolean) => {
        if (first) {
            setLoad(true)
        }
        axios({
